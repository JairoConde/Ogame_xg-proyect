...
$researchTable = DB_PREFIX . 'research';
$columnName = 'research_cargo_optimization';
...
$dbName = DB_NAME;
$checkStmt->bind_param('sss', $dbName, $researchTable, $columnName);
$checkStmt->execute();
...
if ((int) $row['total'] > 0) {
    echo "Column `{$columnName}` already exists in `{$researchTable}`. Nothing to do.\n";
    $mysqli->close();
    exit(0);
}

$alterSql = "ALTER TABLE `{$researchTable}`
             ADD COLUMN `{$columnName}` INT(11) NOT NULL DEFAULT '0'
             AFTER `research_astrophysics`";
...
echo "Migration applied successfully: added `{$columnName}` to `{$researchTable}`.\n";
$mysqli->close();
